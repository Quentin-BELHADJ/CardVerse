                        ...
                        <div class="bg-white p-2 rounded-lg shadow border border-gray-200">
                            <a href="{{ route('cards.show', $card) }}">
                                <img src="{{ $card->image_url ?? 'https://via.placeholder.com/200x280' }}"
                                    alt="{{ $card->name }}" class="rounded-md w-full h-auto block"
                            ...
                            <div class="mt-2 text-center">
                                ...
                                @auth
                                    <form method="POST" action="{{ route('listings.store') }}" class="mt-2"
                                        onsubmit="return confirm('Ajouter à ma collection ?');">
                                        @csrf
                                        <input type="hidden" name="card_id" value="{{ $card->id }}">

                                        <!-- Condition par défaut: Parfait état (ou on pourrait mettre un petit select chiche) -->
                                        <!-- Simplification UX: On ajoute en "Parfait état" par défaut, l'user modifiera après si besoin -->
                                        <!-- Mais la validation demande 'condition', donc input hidden ou select. -->
                                        <!-- Allons pour un select minimaliste -->
                                        <select name="condition" class="text-xs border-gray-200 rounded p-1 w-full mb-1">
