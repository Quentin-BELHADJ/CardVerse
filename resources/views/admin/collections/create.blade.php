<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Ajouter une nouvelle Collection') }}
        </h2>
    </x-slot>
                ...
                <form action="{{ route('admin.collections.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div class="flex items-center gap-4 mt-4">
                            <x-primary-button>{{ __('Créer la collection') }}</x-primary-button>
                            <a href="{{ route('collections.index') }}" class="text-sm text-gray-600 underline">Annuler</a>
                        </div>
                    </div>
                </form>
