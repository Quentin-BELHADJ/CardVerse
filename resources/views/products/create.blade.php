    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                {{-- Le formulaire cible la route products.store --}}
                <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data">
                    @csrf
                    {{-- Pas de @method('PATCH') ou 'DELETE' --}}

                    {{-- 1. Nom --}}
                    <div class="mb-4">
                        <label for="name" class="block font-medium text-sm text-gray-700">Nom du Produit</label>
                        {{-- Utilisation de old() pour préserver la valeur en cas d'erreur de validation --}}
                    ...
                    <div class="mb-4">
                        ...
                        <input id="price" type="number" step="0.01" name="price" value="{{ old('price') }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    ...
                    {{-- 5. Image (File Input) --}}
                    <div class="mb-4">
                        <label for="image" class="block font-medium text-sm text-gray-700">Image du produit</label>
                        {{-- Note : Image est required dans store(), donc pas de 'nullable' --}}
                        <input id="image" type="file" name="image" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm p-1">
